...
const X = {
  count: 30, // Only 3 medals: 1st, 2nd, 3rd place
  colors: [
    0xf9d423, // Gold
    0xc0c0c0, // Silver
    0xcd7f32, // Bronze
  ...
  minSize: 1.2, // Slightly larger medals
  maxSize: 1.5,
  size0: 1.5,
  gravity: 1, //change this
  friction: 0.9975,
  wallBounce: 0.95,
  maxVelocity: 0.15,
  maxX: 5,
...
class Medal extends Group {
  constructor(type, size, color) {
    ...
    // Create medal base (circle)
    const geometry = new CircleGeometry(size, 32);
    ...
    const medalBase = new Mesh(geometry, material);
    medalBase.rotation.x = Math.PI / 2;
    this.add(medalBase);

    // Add number texture for medal type
    const loader = new TextureLoader();
    let numberTexture;

    if (type === 1) {
      numberTexture = loader.load(
        "data:image/svg+xml;base64,"
      );
    } else if (type === 2) {
      numberTexture = loader.load(
        "data:image/svg+xml;base64,"
      );
    } else {
      numberTexture = loader.load(
        "data:image/svg+xml;base64,"
      );
    }
    ...
    const numberMesh = new Mesh(geometry, numberMaterial);
...
class MedalsPit extends d {
  constructor(e, t = {}) {
    const i = { ...X, ...t };
    const s = new z();
    const n = new p(e, 0.04).fromScene(s).texture;
    const o = new CircleGeometry(1, 32); // Flat circle geometry for medals
    ...
    // Create medal objects
    this.medals = [];
    for (let idx = 0; idx < i.count; idx++) {
      const medalType = idx + 1; // 1, 2, 3
      const medalColor = new l(i.colors[idx]);
      const medal = new Medal(medalType, 1, medalColor);
  ...
  update(e) {
    ...
    for (let idx = 0; idx < this.count; idx++) {
      ...
      // Hide the first medal if followCursor is false
      ...
      if (this.medals[idx]) {
        this.medals[idx].position.copy(U.position);
        this.medals[idx].scale.setScalar(this.physics.sizeData[idx]);
      }
    }
...
function createMedalsPit(e, t = {}) {
  ...
  function initialize(e) {
    ...
    s = new MedalsPit(i.renderer, e);
    i.scene.add(s);

    // Add medals to scene
    s.medals.forEach((medal) => {
      i.scene.add(medal);
    });
  }
  i.onBeforeRender = (e) => {
...
const MedalsPitComponent = ({
  className = "",
  followCursor = true,
  ...props
}) => {
  const canvasRef = useRef(null);
  const medalsInstanceRef = useRef(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    if (!canvas) return;

    medalsInstanceRef.current = createMedalsPit(canvas, {
      followCursor,
      ...props,
    });

    return () => {
      if (medalsInstanceRef.current) {
        medalsInstanceRef.current.dispose();
      }
    };
...
export default MedalsPitComponent;
